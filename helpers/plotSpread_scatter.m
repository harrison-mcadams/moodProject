...
switch opt.xyOri
    case 'normal'
        scatter([0.5;nData+0.5],minMax,'o');
    case 'flipped'
        scatter(minMax,[0.5;nData+0.5],'o');
...
for iData = 1:nData
    for iCategory = 1:nCategories
        ...
        if any(currentIdx)
            switch opt.xyOri
                case 'normal'
                    ax = scatter(ah,data(currentIdx,1),...
                        data(currentIdx,2),...
                        opt.markerSize, ... plotColors{iData,iCategory}, ...
                        'MarkerFaceColor',plotColors{iData,iCategory},'MarkerEdgeColor',plotColors{iData,iCategory},...
                        'MarkerFaceAlpha',opt.alpha,'MarkerEdgeAlpha',opt.alpha, ...
                        'marker',plotMarkers{iData,iCategory},...
                        'DisplayName',plotLabels{iData,iCategory});
                case 'flipped'
                    ax = scatter(ah,data(currentIdx,2),...
                        data(currentIdx,1),...
                        opt.markerSize,... plotColors{iData,iCategory}, ...
                        'MarkerFaceColor',plotColors{iData,iCategory},'MarkerEdgeColor',plotColors{iData,iCategory},...
                        'MarkerFaceAlpha',opt.alpha,'MarkerEdgeAlpha',opt.alpha, ...
    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = scatter(ah,opt.xValues,m,'+r','Color','r','MarkerSize',12);
            end
            if any(opt.showMM==[1,3])
                mdh = scatter(ah,opt.xValues,md,'sg','MarkerSize',12);
            end
            if opt.showMM == 4
                mh = scatter(ah,opt.xValues,m,'+r','Color','r','MarkerSize',12);
        ...
        case 'flipped'
            if any(opt.showMM==[1,2])
                mh = scatter(ah,m,opt.xValues,'+r','Color','r','MarkerSize',12);
            end
            if any(opt.showMM==[1,3])
                mdh = scatter(ah,md,opt.xValues,'sg','MarkerSize',12);
            end
            if opt.showMM == 4
                mh = scatter(ah,m,opt.xValues,'+r','Color','r','MarkerSize',12);
